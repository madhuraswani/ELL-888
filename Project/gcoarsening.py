...
def build_coarseGraph(input_features,
               input_graph,
               input_adjacency):
...
def main(argv):
  ...
  model = build_coarseGraph(input_features, input_graph, input_adjacency)
  ...
  np.savetxt("foo.csv",fmt='%s',X=assignments_r,delimiter=',')
  clusters = assignments.argmax(axis=1)
  print(clusters)  # Convert soft to hard clusters.
